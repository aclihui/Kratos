...
function hide($content) {
	if (preg_match_all('/<!--hide start{?([\s\S]*?)}?-->([\s\S]*?)<!--hide end-->/i',$content,$matches)) {
		...
		if ($params['reply_to_this'] == 'true') {
			...
			$tip = '<span style="color:#ff0000">抱歉，只有注册本站账号并对本文发表评论才能阅读隐藏内容。</span>';
		} else {
			if (current_user_can('level_0')) $stats = 'show';
			$tip = '<span style="color:#ff0000">抱歉，只有注册本站账号并在任意文章发表评论才能阅读隐藏内容。</span>';
		}
		$hide_notice = '<span class="hide_notice">'.$tip.'</span>';
		if ($stats == 'show') {
...
function add_plugin($plugin_array) {
    ...
    $plugin_array['hide'] = get_bloginfo('template_url').'/js/buttons/more.js';
